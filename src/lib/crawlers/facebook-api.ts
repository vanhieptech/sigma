...
export async function fetchFacebookPost(postId: string, accessToken: string): Promise<SocialMediaPost> {
...
export async function fetchFacebookComments(
  ...
  try {
    ...
    if (error.message.includes('network') || error.message.includes('timeout')) {
      const backoffTime = Math.min(30000, RATE_LIMIT.DELAY_BETWEEN_REQUESTS * Math.pow(2, Math.floor(Math.random() * 5)));
...
export async function exchangeFacebookAuthCode(code: string): Promise<string> {
  ...
  const response = await fetch(`${GRAPH_API_BASE_URL}/oauth/access_token?client_id=${appId}&redirect_uri=${encodeURIComponent(redirectUri)}&client_secret=${appSecret}&code=${code}`);
...
export async function fetchCommentsForNewPagesExperience(
  ...
  try {
    ...
    commentsEndpoint += '&fields=id,message,created_time,from,attachment,comment_count,like_count,parent';
    ...
    if (commentsData.error) {
      ...
      if (commentsData.error.error_subcode === 2069030) {
        throw new Error("This Page uses the New Pages Experience which has limited API access. Try using a User Access Token with page_read_engagement permission instead.");
      }
...
export async function fetchCommentsAsPage(
  ...
  try {
    ...
    try {
      ...
      const response = await fetch(endpoint + '&fields=id,message,created_time,from,attachment,comment_count,like_count,parent&limit=25' + (cursor ? `&after=${cursor}` : ''));
      const data = await response.json();
      
      if (!data.error) {
        // If successful, map and return the comments
        const comments = (data.data || []).map((comment: any) => mapCommentResponseToSocialMediaComment(comment, postId));
        return {
          comments,
          nextCursor: data.paging?.cursors?.after,
          pageAccessToken
      ...
      if (error.message === "New Pages Experience detected") {
        ...
        return {
          comments: result.comments.map(comment => mapCommentResponseToSocialMediaComment(comment, postId)),
...
export async function getPageAccessTokenForPost(postUrl: string, userAccessToken: string): Promise<string | null> {
  try {
    // Extract the post ID

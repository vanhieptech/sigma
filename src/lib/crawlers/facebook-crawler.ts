...
export class FacebookCrawler {
    ...
    try {
      ...
      while (hasMore && (!options.maxComments || allComments.length < options.maxComments)) {
        try {
          ...
          console.log(`Fetching batch of comments. Current count: ${allComments.length}, Batch size: ${batchSize}`);
          ...
          if (retryCount <= MAX_RETRIES) {
            ...
            console.log(`Max retries reached. Saving ${allComments.length} comments collected so far.`);
            hasMore = false;
          }
        }
      }
  ...
  private generateAnalysis(comments: SocialMediaComment[], post?: SocialMediaPost): EngagementAnalysis {
    // Count comments by author
    const commentsByAuthor = new Map<string, {
      authorId: string;
      authorName: string;
      commentCount: number;
      totalLikes: number;
    ...
    if (post && post.metrics.comments > 0) {
      engagementRate = (post.metrics.likes + post.metrics.comments + post.metrics.shares) / post.metrics.comments;
    }

...
export function FacebookPageCommentsForm() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (data.error?.includes('New Pages experience')) {
          errorMessage = 'This Page uses the New Pages Experience. We\'ve updated our code to handle this, but you may need to get additional permissions or use a different access token. Try refreshing and trying again.';
        } else if (data.error?.includes('OAuthException')) {
          errorMessage = 'Authentication error: Your access token may be invalid or expired. Please try logging in again.';
        } else if (data.error?.includes('permissions error')) {
          errorMessage = 'You don\'t have the required permissions. Make sure you\'ve granted "pages_read_engagement" permission when logging in.';
        }
  ...
  const exportToCsv = () => {
    ...
    // Create CSV content
    const headers = ['ID', 'Author Name', 'Author ID', 'Content', 'Published At', 'Likes', 'Replies', 'Parent Comment ID'];
    const csvContent = [
      headers.join(','),
            ...
            <div className="flex items-center justify-between">
              ...
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent>
                    <p className="max-w-xs">Enter the full URL of a Facebook post from a page you manage</p>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            </div>
              ...
              <div className="space-y-4">
                <div className="bg-muted p-4 rounded-md">
                  <p className="text-sm mb-4">
                    Connect with Facebook to get an access token automatically. This will allow access to comments from pages you manage.
                  </p>
                  <FacebookLoginButton 
                ...
                <div className="flex items-center justify-between">
                  ...
                  <TooltipProvider>
                    <Tooltip>
                      ...
                      <TooltipContent>
                        <p className="max-w-xs">Enter your Facebook access token with pages_read_engagement permission</p>
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                </div>
        ...
        {comments.length > 0 && (
          <div className="mt-8">
            <div className="flex items-center justify-between mb-4">
              ...
              {nextCursor && (
                <Button type="button" variant="outline" onClick={loadMoreComments} disabled={loadingMore}>
                  {loadingMore ? <Spinner className="mr-2 h-4 w-4" /> : <RefreshCw className="mr-2 h-4 w-4" />}
                  Load More
                </Button>
              )}
            </div>
            ...
            <Tabs defaultValue="table">
              ...
              <TabsContent value="table">
                <Table>
                  ...
                  <TableBody>
                    {comments.map(comment => (
                      <TableRow key={comment.id} className={comment.parentCommentId ? "bg-muted/30" : ""}>
                        <TableCell className="font-medium">
                          {comment.authorName}
                          {comment.parentCommentId && <span className="ml-2 text-xs text-muted-foreground">(reply)</span>}
                        </TableCell>

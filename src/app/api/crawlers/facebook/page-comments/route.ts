...
import {
  ...
  getPageAccessTokenForPost,
  fetchCommentsAsPage,
} from '@/lib/crawlers/facebook-api';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!postId) {
      return NextResponse.json({ error: 'Invalid Facebook post URL' }, { status: 400 });
    }
    ...
    try {
      ...
      if (error.message?.includes('New Pages experience')) {
        return NextResponse.json(
          {
            error:
              'This Page uses the New Pages Experience which has some API limitations. ' +

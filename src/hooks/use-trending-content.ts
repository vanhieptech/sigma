...
export function useTrendingContent({
  ...
  const generateContentIdea = useCallback((category: string = selectedCategory): TikTokContentIdea => {
    // Get random template
    const template = ideaTemplates[Math.floor(Math.random() * ideaTemplates.length)];
    
    // Replace category in template
    const displayCategory = category === 'all' ? categories[Math.floor(Math.random() * categories.length)] : category;
    ...
    return {
      ...
      suggestedSoundId: Math.random() > 0.3 ? `sound-${Math.floor(Math.random() * 10) + 1}` : undefined,
      trend: isTrending ? {

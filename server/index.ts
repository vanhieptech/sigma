...
import { AIVoiceManager } from '../src/lib/ai-voice';
import {
  PurchaseEvent,
  StreamEventHandler,
  StreamEventHandlerConfig,
} from '../src/lib/stream-event-handler';
...
app.prepare().then(() => {
  ...
  io.on('connection', socket => {
    ...
    socket.on('purchase', (data: PurchaseEvent) => {
      logger.info(
        'Purchase event',
        `Purchase event from user ${data.uniqueId}: ${data.product.name}`
      );
